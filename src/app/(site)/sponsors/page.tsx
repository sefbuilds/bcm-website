...
export default async function SponsorsPage() {
  const [hoofdsponsors, allPartners] = await Promise.all([
    getSponsors(),
    getPartners(),
  ]);

  const partners = allPartners.filter((p) => p.tier === "partner");
  const vrienden = allPartners.filter((p) => p.tier === "vriend");
  const allNames = [
    ...hoofdsponsors.map((h) => h.company),
    ...allPartners.map((s) => s.name),
  ];

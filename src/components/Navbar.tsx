    ...
    >
      ...
      >
        ...
        >
          ...
          <span className="hidden lg:inline text-[9px] tracking-[0.3em] uppercase text-pearl-60">
            Mallorca
          </span>
        </Link>
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={`inline-flex px-4 py-1.5 rounded-full text-sm transition-all ${
                    active
        ...
        <Link
          href="/lid-worden"
          className="group hidden md:inline-flex items-center gap-1.5 rounded-full bg-terracotta px-5 py-2.5 text-sm font-medium text-white transition-all hover:bg-terracotta-light hover:scale-[1.02]"
        >
          Lid worden
          <ArrowUpRight
            size={14}
        ...
        <button
          type="button"
          className="md:hidden inline-flex items-center justify-center h-10 w-10 rounded-full glass text-pearl"
      ...
      {isOpen && (
        <div
          className="md:hidden fixed inset-x-0 bottom-0 top-[64px] bg-ink z-40 overflow-y-auto"
          role="dialog"
          aria-modal="true"
        >
          <ul className="container-site pt-12 flex flex-col gap-6">
            {NAV_ITEMS.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
